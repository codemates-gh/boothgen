...
export default async function SuperAdminPage() {
  ...
  const [tenants, totalUsers, totalEvents, allSettings] = await Promise.all([
    ...
    prisma.systemSetting.findMany({ where: { key: { in: ['message_retention_months', 'gallery_expire_days', 'gallery_delete_days', 'email_template_welcome', 'email_template_forgot_password', 'stripe_price_monthly_id', 'stripe_price_annual_id'] } } }),
  ]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-8 space-y-8">
        ...
        <PlatformSettings initial={{ message_retention_months: settingsMap.message_retention_months ?? '12', gallery_expire_days: settingsMap.gallery_expire_days ?? '30', gallery_delete_days: settingsMap.gallery_delete_days ?? '30', stripe_price_monthly_id: settingsMap.stripe_price_monthly_id ?? '', stripe_price_annual_id: settingsMap.stripe_price_annual_id ?? '' }} />

          ...
          <div className="flex items-start gap-3 bg-amber-50 border border-amber-300 rounded-xl p-4">
            ...
            <div className="flex-1">
              <p className="font-semibold text-amber-800">
                {needsCountersign.length === 1 ? '1 contract needs your countersignature' : `${needsCountersign.length} contracts need your countersignature`}
              </p>
              <p className="text-sm text-amber-700 mt-0.5">
                {needsCountersign.map(c => `${c.client.firstName} ${c.client.lastName}`).join(', ')} — client{needsCountersign.length !== 1 ? 's have' : ' has'} signed and {needsCountersign.length !== 1 ? 'are' : 'is'} waiting on you.
              </p>
            </div>
          </div>
        )}
          ...
          {contracts.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[560px]">
                ...
                <tbody>
                  {contracts.map(c => (
                    <tr key={c.id} className={`border-b last:border-0 hover:bg-gray-50 ${c.status === 'CLIENT_SIGNED' ? 'bg-amber-50/50' : ''}`}>
                      <td className="px-6 py-4">
                        <p className="font-semibold text-sm">{c.title}</p>
                        {c.event && <p className="text-xs text-gray-400">{c.event.title}</p>}
                      </td>
                      <td className="px-6 py-4 text-sm">{c.client.firstName} {c.client.lastName}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <Badge variant={CC[c.status]}>{c.status.replace(/_/g,' ')}</Badge>
                          {c.status === 'CLIENT_SIGNED' && <PenLine className="w-3.5 h-3.5 text-amber-500" />}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-500">{format(c.createdAt,'MMM d, yyyy')}</td>
                      <td className="px-6 py-4 text-right">
                        <Link href={'/contracts/' + c.id}>
                          <Button variant={c.status === 'CLIENT_SIGNED' ? 'default' : 'ghost'} size="sm">
                            {c.status === 'CLIENT_SIGNED' ? 'Sign Now' : <ArrowRight className="w-4 h-4"/>}
                          </Button>
                        </Link>
                      </td>
                    </tr>

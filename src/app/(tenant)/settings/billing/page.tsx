      ...
      <div className="p-4 sm:p-8 max-w-3xl space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-3"><div><p className="font-semibold">{sub?.plan === 'MONTHLY' ? 'Pro Monthly' : sub?.plan === 'ANNUAL' ? 'Pro Annual' : 'Commission Plan'}</p><p className="text-sm text-gray-500">{sub ? 'Status: ' + sub.status : 'Pay only when you collect — no monthly fee'}</p>{sub?.currentPeriodEnd && <p className="text-xs text-gray-400 mt-0.5">Renews {new Date(sub.currentPeriodEnd).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>}</div><Badge variant={sub?.status === 'ACTIVE' ? 'success' : (!sub || sub.status === 'TRIALING') ? 'default' : 'warning'}>{sub?.status === 'TRIALING' || !sub ? 'ACTIVE' : sub.status}</Badge></div>
            {!sub && <UpgradeButton />}

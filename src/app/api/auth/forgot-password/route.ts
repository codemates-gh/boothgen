...
const DEFAULT_SUBJECT = 'Reset your Booth Genius password';
const DEFAULT_BODY = `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:32px 24px">
...
export async function POST(req: NextRequest) {
  ...
  // Use platform template if customized
  const setting = await prisma.systemSetting.findUnique({ where: { key: 'email_template_forgot_password' } });
  ...
  const vars = { user_name: user.name ?? email, reset_url: resetUrl, app_url: appUrl };

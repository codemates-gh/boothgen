...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { lineItems, notes, terms, taxRatePercent, validUntil, discountCents, discountLabel, couponId, contractTemplateId, paymentType, depositPercent } = body;
  ...
  // Track coupon change: if couponId changed, decrement old and increment new
  const previousCouponId = q.couponId;
  const newCouponId = couponId !== undefined ? (couponId || null) : previousCouponId;
  ...
  const updated = await prisma.quote.update({
    ...
    data: {
      ...
      taxAmountCents: tax, subtotalCents: subtotal, discountCents: discount,
      discountLabel: discountLabel !== undefined ? (discountLabel || null) : undefined,
      couponId: couponId !== undefined ? newCouponId : undefined,
      totalCents: total,
  ...
  if (couponId !== undefined && newCouponId !== previousCouponId) {
    if (previousCouponId) await prisma.coupon.update({ where: { id: previousCouponId }, data: { usedCount: { decrement: 1 } } }).catch(() => {});
    if (newCouponId) await prisma.coupon.update({ where: { id: newCouponId }, data: { usedCount: { increment: 1 } } }).catch(() => {});
  }
